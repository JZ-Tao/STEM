function PSF_l = FE_C(I_MS_LR,I_PAN_LR_C,I_PAN_LR,tap,lambda,mu,th,num_iter,filtername)
...
for jj = 1 : num_iter
    ...
    %%% Estimate alpha
    alpha(1,1,:) = estimation_alpha(cat(3,I_MS_LR,ones(size(I_MS_LR,1),size(I_MS_LR,2))),PAN_LP,'global');
    
    It_E = sum(cat(3,I_MS_LR,ones(size(I_MS_LR,1),size(I_MS_LR,2))) .* repmat(alpha,[size(I_MS_LR,1) size(I_MS_LR,2) 1]),3); 
