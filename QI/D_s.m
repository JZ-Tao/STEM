...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D_s_index = D_s(I_F,I_MS,I_MS_LR,I_PAN,sensorInf,ratio,S,q)
...
% Modified By JZ Tao. 2020
% if size(I_PAN,3) == 1
%     I_PAN = repmat(I_PAN, [1,1,size(I_MS,3)]);
% end
if flag_orig_paper == 0
    %%%%%%% Opt. 1 (as toolbox 1.0) 
    %pan_filt = interp23tap(imresize(I_PAN,1./ratio),ratio); % TJZ
    pan_filt = interpWrapper(imresize(I_PAN,1./ratio),ratio, sensorInf.upsampling);
else
    %%%%%%% Opt. 2 (as paper QNR)
...
%     down_sampling = 0;
% end
% pan_filt =  MTF_conv_sample(I_PAN, sensorInf, ratio, down_sampling);
% imresize(I_PAN,1./ratio);
